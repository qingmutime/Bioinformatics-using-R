setwd("D:\\生信分析数据\\04 OSA DEGs")
rm(list = ls())
library(limma)
library(dplyr)
#以整理好的GSE46960为例
a<-read.csv("OSA array.csv",row.names = 1)#64个患者和14个疾病对照


list <- c(rep("Control", 8), rep("OSA",34)) %>% factor(., levels = c("Control", "OSA"), ordered = F)
list1 <- model.matrix(~factor(list)+0)  #把group设置成一个model matrix
colnames(list1) <- c("Control", "OSA")
df.fit <- lmFit(a, list1)  ## 数据与list进行匹配
df.matrix <- makeContrasts(OSA - Control, levels = list1)
...
library(pheatmap)
x <- t(scale(t(heat)))
pheatmap(x,annotation_col=Groups,fontsize = 4)

#火山图
png(file="火山图_OSA.png",width = 600,height = 600)
